...
class TaskManagerApp:
    ...
    def on_close(self):
        ...
        # No longer need to clean up user task files as we are using SQL Server
...
class TaskManager:
    ...
    def _load_tasks(self) -> List[Dict[str, Any]]:
        ...
        try:
            self.cursor.execute("SELECT * FROM [dbo].[Tasks]") #_load_tasks
    ...
    def _clean_task_files(self):
        ...
        # This method is no longer needed as we are using SQL Server
        pass
    ...
    def delete_task(self, task_id: int) -> bool:
        ...
        try:
            ...
            # Debugging: Check for open transactions before deletion
            logging.info(f"Checking for open transactions before deleting task ID: {task_id}")
            with self.conn.cursor() as check_cursor:
                check_cursor.execute("SELECT * FROM sys.dm_tran_locks WHERE request_status = 'WAIT'")
                locks = check_cursor.fetchall()
                if locks:
                    logging.warning(f"Open transaction detected before deletion: {locks}")

            # SQL query to delete a task
            sql = "DELETE FROM Tasks WHERE id = %s"

            with self.conn.cursor() as cursor:
                logging.info(f"Attempting to delete task ID: {task_id}")  # Log before execution
                cursor.execute(sql, (task_id,))
                self.conn.commit()  # Explicit commit
                logging.info(f"Task ID {task_id} deleted successfully.")  # Log after commit
            ...
            logging.error(f"Error deleting task ID {task_id}: {str(e)}", exc_info=True)
        ...
        try:
            for task_id in task_ids:
                try:
                    ...
                    # Execute the query
                    logging.info(f"Batch deleting task with ID: {task_id}") # ADDED: Logging before batch deletion
                    self.cursor.execute(sql, (task_id,))
                    successful_deletions.append(task_id)
                    logging.info(f"Batch task ID: {task_id} marked for deletion.") # ADDED: Logging after individual deletion attempt
                except pymssql.Error as e:
            ...
            logging.info(f"Batch deletion committed for IDs: {successful_deletions}") # ADDED: Logging after batch commit
            return successful_deletions, failed_deletions
        except pymssql.Error as e:
            logging.error("Error during batch deletion from database", exc_info=True)
            messagebox.showerror("Database Error", f"Error during batch deletion: {str(e)}")
