...
class TaskManager:
    ...
    def __init__(self): # Remove connection_string parameter from __init__
        ...
        self._get_user_credentials()  # Obtain credentials
    ...
    def _connect(self):
        ...
        try:
            ...
            logging.error("Database connection error (pymssql.Error)", exc_info=True)
            error_message = f"Database Connection Error (pymssql):\n\n" \
    ...
    def _load_tasks(self) -> List[Dict[str, Any]]:
        """Load tasks from the SQL Server database, excluding soft-deleted tasks."""
        try:
            sql = "SELECT * FROM [dbo].[Tasks] WHERE is_deleted = 0"  # Add schema and brackets
            ...
            for row in rows:
                task = {
                    "id": row[0],  # Access by index since it's not a named tuple
                    "title": row[1],
                    "description": row[2],
                    ...
                    "qtd_mhr": row[19],
                    "actual_mhr": row[20],
                    "last_modified": row[21].strftime("%Y-%m-%d %H:%M:%S") if isinstance(row[21],
                                                                                             datetime.datetime) else None,
                    "modified_by": row[22],
                    "is_deleted": row[23]  # Add is_deleted to the task dictionary
                }
                tasks.append(task)
    ...
    def add_task(self, title: str, description: str = "", due_date: Optional[str] = None,
        ...
        try:
            ...
            if self._find_task_by_id(new_id):
                ...
                messagebox.showerror("Error", "Failed to add task due to ID conflict. Please try again.",
                                     parent=self.root)
                return  # Abort task addition if ID conflict is detected

            # Get current date and time
            current_datetime = datetime.datetime.now()
            created_date = current_datetime.strftime("%Y-%m-%d") # Format the date
            ...
            sql = """
            INSERT INTO [dbo].[Tasks] (id, title, description, created_date, created_by, due_date, priority, category,
                                main_staff, assigned_to, completed, applied_vessel, rev, drawing_no, link,
                                sdb_link, request_no, requested_date, date_started, qtd_mhr, actual_mhr, last_modified, modified_by)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            """

            # Task data
            task_data = (
                new_id, title, description, created_date, self._get_current_user(), due_date, priority.lower(),
                category,
                main_staff, assigned_to, False, applied_vessel, rev, drawing_no, link,
                sdb_link, request_no, requested_date, date_started, qtd_mhr, actual_mhr, created_date,
                self._get_current_user()
            )
            logging.debug(f"Created date: {created_date}")
    ...
    def add_task_direct_id(self, task_id: int, title: str, description: str = "", due_date: Optional[str] = None,
                     ...
                     link: str = "", sdb_link: str = "", request_no: str = "",
                     requested_date: Optional[str] = None, date_started: Optional[str] = None,
                     qtd_mhr: int = 0, actual_mhr: int = 0, completed: bool = False) -> None:
        ...
        try:
            ...
            logging.debug(f"Parameters: {task_data}")
    ...
    def get_filtered_tasks(self, show_completed: bool = False, category: Optional[str] = None, 
        ...
        try:
            # Build the SQL query
            sql = "SELECT * FROM [dbo].[Tasks] WHERE 1=1"
            params = []
            ...
            messagebox.showerror("Database Error", f"Error getting filtered tasks: {str(e)}")
            return []
    ...
    def update_task(self, task_id: int, **kwargs) -> None:
        ...
        try:
            ...
            # Add last_modified and modified_by to kwargs
            kwargs['last_modified'] = now
            kwargs['modified_by'] = current_user

            # Build the SQL query
            sql = "UPDATE [dbo].[Tasks] SET "  # Add schema and brackets
            params = []
    ...
    def delete_task(self, task_id: int) -> bool:
        """Mark a task as soft-deleted by updating the is_deleted column."""
        try:
            sql = "UPDATE [dbo].[Tasks] SET is_deleted = 1 WHERE id = %s"  # Add schema and brackets
            self.cursor.execute(sql, (task_id,))
            self.conn.commit()
            self.tasks = self._load_tasks()  # Refresh the task list
            return True
        except pymssql.Error as e:
            logging.error("Error soft-deleting task from database", exc_info=True)
            messagebox.showerror("Database Error", f"Error soft-deleting task: {str(e)}")
            return False
    
    def batch_delete_tasks(self, task_ids: List[int]) -> Tuple[List[int], List[int]]:
        """Mark multiple tasks as soft-deleted by updating the is_deleted column."""
        ...
        try:
            for task_id in task_ids:
                try:
                    ...
                    logging.error(f"Error soft-deleting task {task_id} from database", exc_info=True)
            ...
            logging.error("Error during batch soft deletion from database", exc_info=True)
            messagebox.showerror("Database Error", f"Error during batch soft deletion: {str(e)}")
